...
class AdminCog(commands.Cog):
    ...
    def get_filtered_cogs(self, current: str = None):
        ...
        cogs = [f[:-3] for f in os.listdir(cogs_dir) if f.endswith('.py') and f != '__init__.py']
        if current:
    ...
    async def extension_name_autocomplete(self, interaction: discord.Interaction, current: str):
    ...
    async def reload_cog(self, interaction: discord.Interaction, extension_name: str = None, sync: bool = False):
        '''
    ...
    @commands.is_owner()
    @commands.guild_only()
    @app_commands.command(name='turns', description='Generates an ordered list of turns for each player in a role')
    async def turns(self, interaction: discord.Interaction, role: discord.Role):
        ...
        turn_list.extend([f"{i + 1}. {player.mention}" for i, player in enumerate(players)])
    ...
    async def register_user(self, interaction: discord.Interaction, user: discord.Member):
        get_or_create_user(user.id, user.display_name)
        await interaction.response.send_message(f"Registered user: {user.display_name}", ephemeral=True)
